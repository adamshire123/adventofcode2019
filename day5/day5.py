...
def intcode(program: list, input_instruction: int, pointer=0):
    ...
    while not re.match(r"\d{0,3}99$", str(program[pointer])):
        ...
        # parse instruction
        instruction = re.match(r"(\d)??(\d)??(\d)??(\d{0,2})$", str(program[pointer]))
        # put match groups in a dict, use 0 as default if group is empty

        # initialize params list
        params = [0]*4
        instruction_parts = [int(part) for part in instruction.groups(0)]
        opcode = instruction_parts[3]
        
        # operator 1: add
        if opcode == 1:
            # operator takes 3 params
            for i in range(1, 4):
                params[i] = pointer + i if instruction_parts[3-i] == 1 else \
                    program[pointer + i]

            program[params[3]] = program[params[2]] + program[params[1]]
        ...
        if opcode == 2:
            ...
            program[params[3]] = program[params[1]] * \
                program[params[2]]
            # move the pointer ahead 4 places
            pointer = pointer + 4
            continue
        ...
        if opcode == 3:
            ...
            for i in range(1, 2):
                params[i] = pointer + i if instruction_parts[1-i] == 1 else \
                    program[pointer + i]
            program[params[i]] = input_instruction
            pointer = pointer + 2
            continue
